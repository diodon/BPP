...
fileRoot = 'Data/raw/ssp245/'
outFileRoot = 'Data/DHWmax'
...
DHWdoy_4 = getDOY(DHWyear_tmp
, DHWbleach)
DHWdoy_8 = getDOY(DHWyear_tmp
, DHWdead)
DHWdoyrel_4 = getDOYrel(DHWyear_tmp
, DHWbleach, SSTmin)
DHWdoyrel_8 = getDOYrel(DHWyear_tmp
, DHWdead, SSTmin)
DHWndays_4 = getNDays(DHWyear_tmp
, DHWbleach)
DHWndays_8 = getNDays(DHWyear_tmp
, DHWdead)
...
for yy in years[1:]:
    ...
    DHWdoy_4 = xr.concat([DHWdoy_4, getDOY(DHWyear_tmp
, DHWbleach)], 'time')
    DHWdoy_8 = xr.concat([DHWdoy_8, getDOY(DHWyear_tmp
, DHWdead)], 'time')
    DHWdoyrel_4 = xr.concat([DHWdoyrel_4, getDOYrel(DHWyear_tmp
, DHWbleach, SSTmin)], 'time')
    DHWdoyrel_8 = xr.concat([DHWdoyrel_8, getDOYrel(DHWyear_tmp
, DHWdead, SSTmin)], 'time')
    DHWndays_4 = xr.concat([DHWndays_4, getNDays(DHWyear_tmp
, DHWbleach)], 'time')
    DHWndays_8 = xr.concat([DHWndays_8, getNDays(DHWyear_tmp
, DHWdead)], 'time')
...
ds.attrs = {'title': 'DHW general yearly statistics',
            ...
            'time_coverage_start': int(ds.time.min()),
            'time_coverage_end': int(ds.time.max()),
...
ds.to_netcdf(os.path.join(outFileRoot, ('DHW_' + filePrefix)), encoding=encoding)
